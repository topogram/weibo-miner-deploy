...
def setup():
    ...
    run("mkdir -p %(domain_path)s/{releases,shared}" %
        {'domain_path': env.domain_path})
    run("mkdir -p %(shared_path)s/{system,log,index}" %
...
def symlink():
    ...
    run("ln -nfs %(shared_path)s/cdlm.db %(current_release)s/cdlm.db" %
        {'shared_path': env.shared_path, 'current_release': env.current_release})
    run("ln -nfs %(shared_path)s/system/local.py %(current_release)s/%(app_name)s/local.py" %
        {'shared_path': env.shared_path, 'current_release': env.current_release, 'app_name': env.app_name})
    run("ln -nfs %(current_release)s/env/src/django/django/contrib/admin/media %(current_release)s/%(app_name)s/media/admin" %
        {'current_release': env.current_release, 'app_name': env.app_name})
...
def enable():
    ...
    run("rm %(shared_path)s/system/maintenance.html" %
        {'shared_path': env.shared_path})
...
def disable(**kwargs):
    ...
    open("maintenance.html", "w").write(
        render_to_string("maintenance.html", {'now': datetime.datetime.now(
        ), 'deadline': env.deadline, 'reason': env.reason}).encode('utf-8')
    )
    put('maintenance.html', '%(shared_path)s/system/maintenance.html' %
        {'shared_path': env.shared_path})
